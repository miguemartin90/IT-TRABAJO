...
from .models import Colonia, Mision, Planeta
...
class PlanetaForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "nombre": forms.TextInput(attrs={"placeholder": "Kepler-442b"}),
            "tipo": forms.TextInput(attrs={"placeholder": "Rocoso"}),
            "distancia": forms.NumberInput(attrs={"min": 0}),
        }
...
class ColoniaForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "nombre": forms.TextInput(attrs={"placeholder": "Nueva Aurora"}),
            "poblacion": forms.NumberInput(attrs={"min": 0}),
...
class MisionForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "titulo": forms.TextInput(attrs={"placeholder": "Exploracion del sector Orion"}),
    ...
    def clean(self):
        ...
        if fecha and colonia and fecha < colonia.fecha_fundacion:
            self.add_error(
                "fecha",
                "La mision no puede tener una fecha anterior a la fundacion de la colonia.",
            )

        if fecha and estado == Mision.Estado.COMPLETADA and fecha > timezone.localdate():
            self.add_error(
                "estado",
                "Una mision completada no puede tener una fecha futura.",
            )
